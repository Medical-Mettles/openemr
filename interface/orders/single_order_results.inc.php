...
?>
    ...
    <?php if ($genstyles) { ?>
        <style>
            <?php if (empty($_SESSION['language_direction']) || $_SESSION['language_direction'] == 'ltr') { ?>.labres tr.head {
                font-size: 0.8125rem;
                ...
                <tr>
                    ...
                    <td class="border-0 text-right align-top">
                        <?php if ($input_form && !empty($ctx['priors_omitted']) /* empty($_POST['form_showall']) */) { ?>
                            <input type='submit' class='btn btn-primary' name='form_showall' value='<?php echo xla('Show All Results'); ?>' title='<?php echo xla('Include all values reported for each result code'); ?>' />
                        <?php } elseif ($input_form && !empty($_POST['form_showall'])) { ?>
                            <input type='submit' class='btn btn-primary' name='form_latest' value='<?php echo xla('Latest Results Only'); ?>' title='<?php echo xla('Show only latest values reported for each result code'); ?>' />
                        <?php } ?>
                        <?php if (empty($GLOBALS['PATIENT_REPORT_ACTIVE'])) { ?>
                            &nbsp;
                            <input type='button' class='btn btn-primary' value='<?php echo xla('Related Patient Notes'); ?>' onclick='showpnotes(<?php echo attr_js($orderid); ?>)' />
                        <?php } ?>
                        <?php if ($input_form && $ctx['sign_list']) { ?>
                            ...
                            <input type='submit' class='btn btn-primary' name='form_sign' value='<?php echo xla('Sign Results'); ?>' title='<?php echo xla('Mark these reports as reviewed'); ?>' />
                        <?php } ?>
                        <?php if ($input_form) { ?>
                            &nbsp;
