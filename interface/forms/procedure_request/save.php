...
if ($id && $id != 0) {
    sqlStatement("DELETE FROM `form_procedure_request` WHERE id=? AND pid = ? AND encounter = ?", array($id, $_SESSION["pid"], $_SESSION["encounter"]));
    $newid = $id;
} else {
    $res2 = sqlStatement("SELECT MAX(id) as largestId FROM `form_procedure_request`");
    ...
    addForm($encounter, "Procedure Request Form", $newid, "observation", $_SESSION["pid"], $userauthorized);
}
...
if (!empty($code_desc)) {
    foreach ($code_desc as $key => $codeval) :
        ...
        $sets = "id     = ?,
            ...
            date        = ?";
        sqlStatement(
            "INSERT INTO form_procedure_request SET $sets",
            [
                $newid,
                $_SESSION["pid"],
